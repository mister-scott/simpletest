"""
GUI executor component for SimpleTest.
Manages the main GUI window and test execution interface.
"""
...
class TestExecutor:
    ...
    def create_gui(self, version: str) -> None:
        """
    ...
    def load_test_series(self, test_series_file: str) -> None:
        """
        Load a specific test series file.
        
        Args:
            test_series_file: Path to test series file
        """
        if self.file_manager.set_test_directory(test_series_file):
            self.test_runner.load_tests()
            self.update_test_list()
            self.status_bar.set_test_series(self.file_manager.get_test_series_file())
